...
%             mpas.field = squeeze(ncread(data_fi, F{1}, [run(i).levelp1,1,t_ind], [1,Inf,1]));
            mpas.field = ncread(data_fi, F{1}, [1,1,t_ind], [Inf,Inf,1]);
            mpas.field = squeeze(mean(mpas.field, 1, 'omitnan'));
...
for plot_name = {'vert_mean'}
    ...
    if w_mmap
        ...
        if strcmp(p,'btspeed_mean') | strcmp(p,'btspeed_snap') | strcmp(p,'vort_snap') | strcmp(p,'vort_mean') | strcmp(p,'vgeo_snap') | strcmp(p,'vert_mean')
            m_contourf(run(i).LON, run(i).LAT, data, bins,'linecolor','none')
        else
            m_contourf(run(i).LON, run(i).LAT, data, bins,'linecolor','k')
        end
...
%         end
        ...
        contourf(run(i).LON, run(i).LAT, data, bins,'linestyle','none') 
...
%         end
    ...
    save_name = sprintf('figures/upwelling/vertical_vel_depthmean_draft0.png');%, region, p, version_code);
