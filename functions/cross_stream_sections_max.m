function [max_speed, angle, lon, lat] = cross_stream_sections_max(mesh_fi, data_fi, cont, lon_vec, lat_vec, width)
...
% v3:
% May 20, 2019
% - want lat/lon of contour points. algorithm:
%   * call contour() twice: once on xy [curvi] and once on lonlat [mesh]
%   * contour lines should be the same length since they are acting on
%     same-sized matrices but check to be sure.
%   * the contour locations (both xy and latlon) should then remain valid
%     for the xy meshgrid that will be generated.
...
[SSH.x1, SSH.y1, SSH.field1, SSH.lon, SSH.lat] = read_and_interp_new_field(field_to_read, mesh_fi, data_fi, t_ind, lon_vec, lat_vec);

% generate contour line
[C.x, C.y] = contour_line_edgeworkaround(SSH.x1, SSH.y1, SSH.field1, cont);
[C.lon, C.lat] = contour_line_edgeworkaround(SSH.lon, SSH.lat, SSH.field1, cont);

if length(C.x) ~= length(C.lon)
    error('Contour different lengths in xy and latlon projections')
end

[SSH.x, SSH.y, SSH.field] = curv_to_meshgrid(SSH.x1, SSH.y1, SSH.field1);  % is this step actually necessary?
...
[U.x1, U.y1, U.field1] = read_and_interp_new_field(field_to_read, mesh_fi, data_fi, t_ind, lon_vec, lat_vec);
[U.x, U.y, U.field] = curv_to_meshgrid(U.x1, U.y1, U.field1);

U.field(U.field<0) = 0;
U.field = sqrt(2 * U.field);


%% Max speed across streamline
N = 100;  % integration points

max_speed = NaN(size(angle));
...
inds = ~isnan(max_speed);

max_speed = max_speed(inds);
angle = angle(inds);
lon = C.lon(inds)';
lat = C.lat(inds)';
...
function [x, y, field, lon, lat] = read_and_interp_new_field(field_to_read, mesh_fi, data_fi, t_ind, lon_vec, lat_vec)
