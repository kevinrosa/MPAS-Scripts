...
%     no_years = length(files)/12;
    t_length = length(months); %floor(t_inds_per_year * no_years);  % approx number of time indices -- used to initialize arrays 
...
% SAVE
save_name = sprintf('figures/mld/northatlantic_salt_%s_month%i_%s.png', run(i).short_name, t, version_code);
set(gca,'color','none');
...
save_name = sprintf('figures/mld/northatlantic_saltmap_colorbar_%s.png', version_code);
% set(gca,'color','none');
export_fig(save_name, '-m3','-painters','-transparent')
