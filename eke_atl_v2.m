...
if w_mmap == 0
    ...
    m_grid('box','fancy','tickdir','out','fontsize',14)
...
save_name = sprintf('figures/mapviews/map_%s_%s_%s_%s', region, FIELD, run(i).short_name, version_code);
set(gca,'color','none');
...
cb = colorbar;
ylabel(cb, 'Surface eddy kinetic energy (cm2 s-2)')
set(cb,'fontsize',14)
set(gca,'visible','off')

save_name = sprintf('figures/mapviews/map_%s_%s_colorbar_%s', region, FIELD, version_code);
