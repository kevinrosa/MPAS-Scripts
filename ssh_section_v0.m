...
%     run(i).speed = NaN([t_length, length(lon_vec), length(lat_vec)]);
    run(i).ssh = NaN([t_length, length(lon_vec), length(lat_vec)]);

    tt = 1;
    for m = 1:length(files)

        [LON, LAT, ssh] = mpas_to_lonlat_meshgrid('ssh', run(i).mesh_fi, files{m}, lon_vec, lat_vec, t_ind);
%         [LON, LAT, ssh] = mpas_to_lonlat_meshgrid('temperatureAt250m', run(i).mesh_fi, files{m}, lon_vec, lat_vec, t_ind);
...
%% apply mask nans for ssh
for i = 1:length(run)
    run(i).ssh = run(i).ssh .* repmat(mask, [length(run(i).time), 1,1]);
end
...
for i = 1:length(run)
    ...
    run(i).ssh(run(i).ssh<-100) = NaN;
    
    imagesc(LON, run(i).time/364.25, run(i).ssh)
    colormap(jet)
    daspect([35 10 1])
    xlabel('Longitude (^o East)')
    ylabel('Time (years)')
    title(sprintf('SSH at %.1f^o N: %s', LAT(1), run(i).name))
    set(gca,'fontsize',14)
    
%     saveas(gcf,sprintf('figures/westward_propogation/imagesc_%s_00v0.png',run(i).short_name))
end
...
for i = 1:length(run)
    ...
    normalize = repmat(mean(run(i).ssh,2,'omitnan'), [1,length(run(i).ssh(1,:))]);
    imagesc(LON, run(i).time/364.25, run(i).ssh - normalize)
    colormap(jet)
    daspect([35 10 1])
    xlabel('Longitude (^o East)')
    ylabel('Time (years)')
...
for t = 1:length(run(i).time)
    set(ll,'visible','off')
    ll = line(LON, run(i).ssh(t,:),'color','k');
    ylim([-2 0.5])
    pause(0.1)
end
...
%%
figure
imagesc(run(i).time, LON, run(i).ssh')
set(gca,'ydir','normal')
