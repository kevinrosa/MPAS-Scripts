...
%     pcolor(squeeze(run(i).LON(:,eta)), time, ssh(:,:,eta) - time_mean); shading flat
    ...
    position = time_sec * ref_speed * 1e-3;
%     line(position, time, 'linewidth',2,'color',0.3*[1,1,1]);
    ...
    save_name = sprintf('figures/westward_propogation/hovmoeller_cal_%s_v2.png',run(i).short_name);
    set(gca,'color','none');
...
save_name = 'figures/westward_propogation/hovmoeller_cal_colorbar_v2.png';
export_fig(save_name, '-m4','-painters','-transparent')