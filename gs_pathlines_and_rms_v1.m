...
for i = 2:3
    figure
    set(gcf,'color','w','position',[560 420 683 506])
    ...
    if i == var_res_ind
        m_contour(run(i).LON, run(i).LAT, run(i).widthCell, transition_contour*[1,1], 'linecolor','k','linewidth',2)
    end

    colormap(cmap)
    caxis(crange)
    
    save_name = sprintf('figures/gulfstream/gs_rms_%s_v1.png', run(i).short_name);
    set(gca,'color','none');
...
cb = colorbar;
ylabel(cb, 'Surface speed RMS (m/s)', 'fontsize',12)
cbarrow

save_name = 'figures/gulfstream/gs_rms_colorbar_v1.png';
set(gca,'color','none');
export_fig(save_name, '-m3','-painters','-transparent')
...
save_name = 'figures/gulfstream/gs_paths_v1.png';
set(gca,'color','none');
export_fig(save_name, '-m3','-painters','-transparent')
