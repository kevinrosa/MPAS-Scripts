...
run(i).dir = sprintf('/scratch/kanga/runs/%s/mpaso.hist.am.highFrequencyOutput/',run(i).code);
run(i).mesh_fi = sprintf('/scratch/kanga/runs/%s/ocean.oRRS18to6v3.scrip.181106.nc',run(i).code);
...
%         
%         run(i).ssh_mean(tt) = mean_along_section('ssh', run(i).mesh_fi, data_fi, section_lon, section_lat, t_ind);
%         
...
%     
%     save_name = sprintf('ssh_mean_lat%.1f_%s_v0.mat',section_lat(1),run(i).code);
...
for i = 1:length(run)
    ...
    for m = 1:length(files)
        ...
        run(i).ssh_mean(tt) = mean_along_section('ssh', run(i).mesh_fi, data_fi, section_lon, section_lat, t_ind);
        run(i).ssh(tt,:,:)  = ssh - run(i).ssh_mean(tt);
