...
%%
FIELDS = {'dThreshMLD','salinityAtSurface'};
t_ind = 'all';
months = 1:12;
...
%     no_years = length(files)/12;
    t_length = length(months) * length(run(i).years) * 5; %floor(t_inds_per_year * no_years);  % approx number of time indices -- used to initialize arrays 
...
dc = 0.25;
bins = 0:dc:crange(2);
cmap = cmocean('haline', length(crange(1):dc:crange(2))-1);

% show a contour around the salty region
s_cont = 34.5;
...
if w_mmap
    ...
    contourf(run(i).LON, run(i).LAT, data, bins,'linecolor','none')
end
hold on
% add transition region contour
if i == var_res_ind
    if w_mmap
        m_contour(run(i).LON, run(i).LAT, run(i).widthCell .* run(i).mask, transition_contour*[1,1],'linecolor',0.2*[1,1,1],'linewidth',4)
        m_contour(run(i).LON, run(i).LAT, run(i).widthCell .* run(i).mask, transition_contour*[1,1],'linecolor',1.0*[1,1,1],'linewidth',2)
    else
        contour(run(i).LON, run(i).LAT, run(i).widthCell .* run(i).mask, transition_contour*[1,1],'linecolor',0.6*[1,1,1],'linewidth',2)
    end
end

if w_mmap
    m_contour(run(i).LON, run(i).LAT, data, s_cont*[1,1],'linecolor',rgb('red'),'linewidth',2)
    
    m_gshhs_i('patch',0.8*[1,1,1],'edgecolor','k');
    m_grid('box','fancy','tickdir','out','fontsize',12)
else
    contour(run(i).LON, run(i).LAT, data, s_cont*[1,1],'linecolor',rgb('red'),'linewidth',2)
end
...
% % SAVE
save_name = sprintf('figures/mld/northatlantic_salt_%s_month%i_%s', run(i).short_name, months, version_code);
set(gca,'color','none');
...
save_name = sprintf('figures/mld/northatlantic_saltmap_colorbar_%s', version_code);
export_fig(save_name, '-pdf','-painters','-transparent')

%% save just colorbar - horizontal
figure 
set(gcf,'color','w','position',[560 547 461 379])
colormap(cmap)
caxis(crange)

cb = colorbar;
ylabel(cb, 'Salinity (PSU)')
set(cb,'ydir','normal','fontsize',14,'location','southoutside')
set(gca,'visible','off')
% cbarrow

save_name = sprintf('figures/mld/northatlantic_saltmap_colorbar_horizontal_%s', version_code);
...
% SAVE
save_name = sprintf('figures/mld/northatlantic_mld_%s_%s', run(i).short_name, version_code);
set(gca,'color','none');
export_fig(save_name, '-m3','-painters','-transparent','-nocrop')
...
cb = colorbar;
ylabel(cb, 'Mixed layer depth (m)')
set(cb,'ydir','reverse','fontsize',14)
set(cb,'ydir','normal','fontsize',14)
set(gca,'visible','off')

save_name = sprintf('figures/mld/northatlantic_mldmap_colorbar_ydirnormal%s', version_code);
export_fig(save_name, '-m3','-painters','-transparent')
...
save_name = sprintf('figures/mld/northatlantic_mldmap_colorbar_horizontal_%s', version_code);
export_fig(save_name, '-m3','-painters','-transparent')
