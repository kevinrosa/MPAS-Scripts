...
DIR = '~/data/mesh_sensitivity/CUSP8/';
mesh_fi = fullfile(DIR, 'init.nc');
data_fi = fullfile(DIR, '/mpaso.hist.am.highFrequencyOutput.0004-01-01_00.00.00.nc');
...
ssh_contour = -0.2;
t_ind = 1;
% [LON, LAT, KE] = mpas_to_lonlat_meshgrid('kineticEnergyAtSurface',mesh_fi,data_fi,lon_vec,lat_vec,t_ind);
[LON, LAT, SSH] = mpas_to_lonlat_meshgrid('ssh',mesh_fi,data_fi,lon_vec,lat_vec,t_ind);
...
% c = contour(LON, LAT, SSH, ssh_contour*[1,1]);
c2 = contourc(lon_vec, lat_vec, SSH', ssh_contour*[1,1]);

%%
line(c2(1,2:end), c2(2,2:end),'color','k')
